...
export default function EditModal({ drink, onClose, onSave }) {
    ...
    const PRESETS = [2, 4, 8, 12];
    const isPreset = PRESETS.includes(volume);
        ...
        <>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '2rem' }}>
                        ...
                        <button
                            ...
                            style={{
                                background: !isPreset ? '#fbb124' : '#333',
                                color: !isPreset ? 'black' : '#888',
                                border: 'none', padding: '10px 0', borderRadius: '8px', fontWeight: 'bold',
                                fontSize: '0.75rem'
                            }}
                        >
            ...
            {showVolumeSelector && (
                <CustomVolumeSelector
                    volume={volume}
                    onSelect={(v) => { setVolume(v); setShowVolumeSelector(false); }}
